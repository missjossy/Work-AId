...
AND date(sa.creationdate) between '2025-04-03' and '2025-09-16'
--and cl.id =  '304489447'
),
...
ln_at_signup_calc AS (
  ...
  MAX(CASE 
    ...
    END) as ln_at_signup
  FROM (
    SELECT DISTINCT client_id, first_creation_date
...
active_accounts_by_period as (
    select 
        ...
        case when ad.ln is null and ld.score < 250 then 'Ineligible New' 
        when ad.ln >=0 and ld.score < 250 then 'Returning Ineligible'
        when ad.ln is null and ld.score >= 250 then 'Eligible New'
    ...
    from periods p
    cross join account_details ad
    left join loan_details ld on ad.client_id = ld.client_id
    where 
...
select 
    ...
    -- Cumulative count by LN Group
    SUM("Active Accounts") OVER (
        PARTITION BY "LN Group"
        ORDER BY period 
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
    ) as "Cumulative Count by LN Group",
    -- Total cumulative count across all groups
    SUM("Active Accounts") OVER (
        ORDER BY period 
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
    ) as "Total Cumulative Count"
