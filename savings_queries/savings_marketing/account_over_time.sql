WITH deposits_over_time AS (
    SELECT 
        date(st.entrydate) as day_of_month, 
    ...
    AND st."type" = 'DEPOSIT'
    AND date(st.creationdate) BETWEEN date('{{Range.start}}') and date('{{Range.end}}')
...
accounts_created_over_time AS (
    SELECT 
        date(sa.creationdate) as day_of_month,
...
first_account_creation AS (
    SELECT 
        date(first_creation_date) as day_of_month,
...
--FROM all_days ad
FROM deposits_over_time dot --ON ad.day_of_month = dot.day_of_month
LEFT JOIN accounts_created_over_time acot ON dot.day_of_month = acot.day_of_month
LEFT JOIN first_account_creation fac ON dot.day_of_month = fac.day_of_month
ORDER BY dot.day_of_month