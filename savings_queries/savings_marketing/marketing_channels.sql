...
,cost_data AS (
SELECT date_trunc('month',cost_date::date) as time ,network_partner_name, partner_campaign_name ,partner_campaign_id, sum(total_spend_USD) daily_cost FROM (
SELECT DISTINCT * FROM ghana_prod.kochava_data.cost_details_view)
GROUP BY 1,2,3,4)            
 
,signups as (
select date_trunc('month',CREATED_TIMESTAMP::date) as time,ATTRIBUTION_NETWORK_NAME,
