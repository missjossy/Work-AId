-- Marketing Source Summary: Web2App vs Google vs Meta
-- Shows overall performance by source for Easysave campaigns
...
SELECT 
    ...
    -- Additional metrics
    COUNT(DISTINCT COALESCE(ms.campaign_name, at.campaign_name)) as unique_campaigns,
    MIN(COALESCE(ms.signup_date, at.signup_date)) as earliest_signup,
    MAX(COALESCE(ms.signup_date, at.signup_date)) as latest_signup
FROM all_traffic at
LEFT JOIN marketing_savings ms ON at.source = ms.source
GROUP BY COALESCE(ms.source, at.source)
ORDER BY total_savings_signups DESC;
