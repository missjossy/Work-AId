...
new_savers AS ( 
SELECT 
to_char(date_trunc('{{Scale}}', first_creation_date), 'YYYY-MM-DD') date_ , 
...
returning_savers AS (
SELECT 
to_char(date_trunc('{{Scale}}', creationdate), 'YYYY-MM-DD') date_ , 
...
clients_with_loans AS (
    -- Clients who have ever taken a loan (disbursed)
    SELECT DISTINCT 
        ml.CLIENT_KEY as client_key,
        ml.CLIENT_ID as client_id
    FROM GHANA_PROD.ML.LOAN_INFO_TBL ml
    WHERE ml.DISBURSEMENTDATE IS NOT NULL
),

ln0_loans_without_savings AS (
    -- Clients who took LN0 loan but don't have a savings account
    SELECT 
        to_char(date_trunc('{{Scale}}', ml.DISBURSEMENTDATE), 'YYYY-MM-DD') as date_,
        COUNT(DISTINCT ml.CLIENT_KEY) as ln0_loan_only_clients
    FROM GHANA_PROD.ML.LOAN_INFO_TBL ml
    LEFT JOIN (
    ...
    ) savings_clients ON ml.CLIENT_ID = savings_clients.client_id
    WHERE ml.LN = 0
        AND ml.DISBURSEMENTDATE IS NOT NULL
        AND DATE(ml.DISBURSEMENTDATE) BETWEEN date('{{Range.start}}') and date('{{ Range.end }}')
        AND savings_clients.client_id IS NULL  -- No savings account
    GROUP BY 1
    ORDER BY 1
),
...
active_accounts_by_date as (
    ...
    JOIN GHANA_PROD.MAMBU.SAVINGSACCOUNT sa ON 
        ...
        AND (
            ...
            OR 
            (sa.ACCOUNTSTATE = 'CLOSED' AND DATE(sa.closeddate) > d.date_)
        )
    LEFT JOIN MAMBU.CLIENT cl on cl.ENCODEDKEY = sa.ACCOUNTHOLDERKEY
    LEFT JOIN GHANA_PROD.MAMBU.CUSTOMFIELDVALUE wallet_id ON 
        sa.ENCODEDKEY = wallet_id.PARENTKEY AND 
...
active_users_by_period as (
    SELECT 
        TO_CHAR(DATE_TRUNC('{{Scale}}', report_date), 'YYYY-MM-DD') AS date_,
...
select total_unique_attempts.date_,
    DAYNAME(total_unique_attempts.date_) as weekday,
       ...
       active_users_by_period.active_accounts as total_active_saving, 
       new_savers.new_daily_clients as new_saving_clients ,
       returning_savers.returning_daily_clients as returning_clients,
...
full join biz_doc on biz_doc.date_=total_unique_attempts.date_ 
 join new_savers on new_savers.date_ = total_unique_attempts.date_
 join returning_savers on returning_savers.date_ = total_unique_attempts.date_
full join active_users_by_period on active_users_by_period.date_ = total_unique_attempts.date_ 
full join savings_without_loans_by_period on savings_without_loans_by_period.date_ = total_unique_attempts.date_
full join cumulative_signups on cumulative_signups.date_ = total_unique_attempts.date_
full join ln0_loan_only_daily on ln0_loan_only_daily.date_ = total_unique_attempts.date_
full join ln0_loan_only on ln0_loan_only.date_ = total_unique_attempts.date_
