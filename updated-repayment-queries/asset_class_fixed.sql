...
SELECT
    ...
    -- Calculate asset classification directly in main query
    CASE
        WHEN ml.accountstate = 'ACTIVE' THEN 'A'
        WHEN ml.accountstate = 'CLOSED_WRITTEN_OFF' THEN 'E'
    ...
    IFF(ml.accountstate = 'ACTIVE_IN_ARREARS',
        DATEDIFF('day', ml.lastSetToArrearsDate, CURRENT_DATE()),
        NULL) as "NDIA",
    ...
    CASE
        ...
        WHEN ml.accountstate = 'CLOSED_WRITTEN_OFF' THEN 'W'
        ELSE 'A'
...
AND ml.accountstate IN ('ACTIVE', 'ACTIVE_IN_ARREARS')  -- Filter for A and C classifications
