CREATE OR REPLACE VIEW mags_report_view AS
...
mlpscl AS (
    SELECT 
        ...
        MONTH as eomonth,
        datediff(days, m.last_expected_repayment, least(MONTH, CURRENT_DATE)) as days_late, 
        par,
        l.parentaccountkey,
        l.principalbalance,
        CASE 
            WHEN l.principalbalance < m.loanamount THEN l.principalbalance 
            ELSE m.loanamount 
        END new_balance,
        l.balance,
        m.loan_id,
        ...
        CASE 
            WHEN l."type" in ('REPAYMENT', 'REPAYMENT_ADJUSTMENT') THEN l.amount 
            ELSE 0 
        END repaid,
    ...
    LEFT JOIN (
        SELECT monthb,
               loan_id,
               min(earliest_open_installment_duedate) AS earliest_open_installment_duedate,
               max(par) AS par
        FROM (
            SELECT monthb,
                   loan_id,
                   TOTALDUE AS TOTAL_DUE,
                   TOTAL_REPAYMENT_AMOUNTT,
                   min(CASE
                       WHEN TOTAL_REPAYMENT_AMOUNTT::float < TOTALDUE::float THEN REPAYMENT_DUE_DATE
                   END) earliest_open_installment_duedate,
                   datediff('day',earliest_open_installment_duedate, least(monthb,CURRENT_DATE)) AS par,
                   min(CASE
                       WHEN TOTAL_REPAYMENT_AMOUNTT::float < TOTALDUE::float THEN installment
                   END) earliest_open_installment_num
            FROM (
                SELECT MONTH AS monthb,
                       rt.LOAN_KEY,
                       rt.loan_id,
                       installment,
                       REPAYMENT_DUE_DATE,
                       ...
                       nvl(sum(CASE
                           WHEN last_day(rt.TRANSACTION_DATE) <= monthb THEN rt.amount
                           ELSE 0
                       END),0) TOTAL_REPAYMENT_AMOUNTT,
                       round(TOTALDUE,2) - round(TOTAL_REPAYMENT_AMOUNTT,2) AS balance
                ...
                WHERE CASE
                    WHEN l.LOAN_PRODUCT_ID LIKE '%UCBLL%' THEN 'FidoBiz'
                    ELSE 'General'
                END IN ('General', 'FidoBiz')
                AND installment > 1
        ...
        GROUP BY 1, 2
    ) par ON par.loan_id = m.loan_id
    WHERE date(l.creationdate) <= CURRENT_DATE
)
SELECT 
    client_mambu_id AS "Customer ID",
    ...
    CASE 
        WHEN ("Days in Arrears" <= 60 OR "Days in Arrears" IS NULL) THEN 'Stage 1' 
